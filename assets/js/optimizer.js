...
jQuery(window).ready(function() {
	...
	if (jQuery(window).width() > 768) {
		...
		jQuery('.search-toggle').hover(function(){
				jQuery(this).addClass('search_hover');
			}, function(){
				jQuery(this).removeClass('search_hover');	
			});

		//Top Menu Search Form Show & Hide
		jQuery(function() {
			var toggle = jQuery('.search-toggle');
			var former = jQuery('.menu-searchform');
			var fade   = jQuery('.search-menu-fade');

			toggle.on('click', function(e){
		      	if(former.hasClass('hide-menu-searchform')) {
		     		former.removeClass('hide-menu-searchform');
		     		fade.addClass('open');
		     		toggle.addClass('click_search');
		      	} else {
		        	former.addClass('hide-menu-searchform');
		        	fade.removeClass('open');
		        	toggle.removeClass('click_search');
		      	}
		      	e.stopPropagation();
		  	});

		  	fade.on('click', function(e) {
			    if (jQuery(e.target).is(toggle || former) == false) {
			        fade.removeClass('open');
			        toggle.removeClass('click_search');
			        former.addClass('hide-menu-searchform');
			        e.stopPropagation();
